<template>    
    <v-card flat min-height=300 class="mt-10">
        <v-card-title class="pa-6">
            <span class="semibold-24">Units</span>
        </v-card-title>
        <v-card-text>    
            ...
            <div v-for="item in units" :key="item.id" class="clearfix float-none">
    ...
    export default {
        ...
        methods: {
            agree() {
                ...
                this.axios
                    .delete(`/api/settings/units/${this.deletedId}`)
                    .then(response => {
                        this.allItem();
                    });  
            ...
            editItem(item) {
                ...
                if(this.formData.image != null)
                    this.file_url = location.host + '/images/' + this.formData.image;
            },
            deleteItem(id) {                
            ...
            async saveForm() {
                ...
                if (valid)
                {
                    let call_url = '/api/settings/units';
